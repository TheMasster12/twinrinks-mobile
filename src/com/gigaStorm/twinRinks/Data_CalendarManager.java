...
public class Data_CalendarManager {
	...
	private void loopThroughGames(int whichCalendar) {
		...
		for (Model_Game e1 : games)
			for (Model_Team e : memoryManager.getYourTeams())
				if ((e1.getTeamA().equalsIgnoreCase(e.getTeamName()) || e1
						.getTeamH().equalsIgnoreCase(e.getTeamName()))
						&& e1.getLeague().equalsIgnoreCase(e.getLeague()))
					if (!e1.hasPassed())
						addGameToCalendar(e1, whichCalendar);
	}
	...
	private void addGameToCalendar(Model_Game game, int whichCalendar) {
		...
		try {
			values.put(CalendarContract.Events.CALENDAR_ID, whichCalendar + 1);
			values.put(CalendarContract.Events.TITLE,
					"Hockey- " + game.getLeague() + ": " + game.getTeamH()
							+ " vs " + game.getTeamA());
			values.put(CalendarContract.Events.EVENT_LOCATION,
					"Twin Rinks Ice Arena - " + game.getRink() + " Rink");
			values.put(CalendarContract.Events.DTSTART, game
					.getCalendarObject().getTimeInMillis());
	...
	public void toast(Object obj) {
		Toast toast = Toast
				.makeText(context, obj.toString(), Toast.LENGTH_LONG);
		toast.show();
	}
}